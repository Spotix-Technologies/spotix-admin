...
/**
 * app/admin-dashboard/event-data/addons-tab.tsx
 *
 ...
 * /api/v1/event-data/addons; see that route's header comment for why the
 * customer-support dashboard has its own parallel version rather than
 * sharing this one.
 *
 * Creating and deactivating addons is full-admin only (canCreate), same
 * tier as pricing edits — everyone with the Event Data tab can still view
 * the list.
 */
...
export default function AddonsTab({ eventId, canCreate = false }: { eventId: string; canCreate?: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/v1/event-data/addons?eventId=${eventId}`)
  ...
  const handleCreate = async () => {
    ...
    setCreating(true)
    try {
      const res = await fetch("/api/v1/event-data/addons", {
        method: "POST",
