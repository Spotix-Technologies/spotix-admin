...
/**
 * app/components/payout/admin-poll-payouts-panel.tsx
 *
 * Poll equivalent of admin-event-payouts-panel.tsx — same shape, reads
 * from the admin-polls endpoints, no Vault feature so no override path.
 * Shared by both role dashboards that expose Voting Control (admin,
 * customer-support).
 */
...
export default function AdminPollPayoutsPanel({
  pollId, adminUsername, canManage,
}: { pollId: string; adminUsername: string; canManage: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [txRes, poRes] = await Promise.all([
        fetch(`/api/v1/admin-polls?pollId=${pollId}&action=transactions`),
        fetch(`/api/v1/admin-polls?pollId=${pollId}&action=payouts`),
      ])
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        {transactions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 gap-3 text-slate-400 bg-slate-50 border border-slate-200 rounded-xl">
            ...
            <p className="text-sm">No transaction dates recorded for this poll yet.</p>
          </div>
        ) : (
                ...
                <div key={t.date} className="flex items-center justify-between px-4 py-3 gap-3 flex-wrap">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {t.voteCount ?? 0} vote{(t.voteCount ?? 0) !== 1 ? "s" : ""} · ₦{Number(t.voteSales ?? 0).toLocaleString()}
                    </p>
                  </div>
                  {blocking && cfg ? (
      ...
      <div>
        ...
        {payouts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 gap-3 text-slate-400 bg-slate-50 border border-slate-200 rounded-xl">
            <Wallet className="w-8 h-8 text-slate-300" />
            <p className="text-sm">No payouts found for this poll.</p>
          </div>
        ) : (
