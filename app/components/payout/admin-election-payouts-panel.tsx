...
/**
 * app/components/payout/admin-election-payouts-panel.tsx
 *
 * Election equivalent of admin-poll-payouts-panel.tsx — same shape,
 * reads from the nested admin-elections/[electionId]/payouts endpoint
 * (see that route's header comment for why elections get one combined
 * file instead of flat query-param routes like polls).
 */

import { useState, useEffect, useCallback } from "react"
import { Wallet, Loader2, AlertCircle, XCircle, CheckCircle2, ShieldAlert, RotateCcw } from "lucide-react"
...
export default function AdminElectionPayoutsPanel({
  electionId, adminUsername, canManage, suspended,
}: { electionId: string; adminUsername: string; canManage: boolean; suspended: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [txRes, poRes] = await Promise.all([
        fetch(`/api/v1/admin-elections/${electionId}/payouts?action=transactions`),
        fetch(`/api/v1/admin-elections/${electionId}/payouts?action=payouts`),
      ])
    ...
    <div className="space-y-6">
      ...
      {suspended && (
        <div className="flex items-start gap-2.5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
          ...
          This election is suspended — unsuspend it before recording any payout.
        </div>
      )}
      ...
      <div>
        ...
        {transactions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 gap-3 text-slate-400 bg-slate-50 border border-slate-200 rounded-xl">
            ...
            <p className="text-sm">No transaction dates recorded for this election yet.</p>
          </div>
        ) : (
                ...
                <div key={t.date} className="flex items-center justify-between px-4 py-3 gap-3 flex-wrap">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {t.formCount ?? 0} form{(t.formCount ?? 0) !== 1 ? "s" : ""} · ₦{Number(t.totalAmount ?? 0).toLocaleString()}
                    </p>
                  </div>
                  {blocking && cfg ? (
                    ...
                    <button
                      ...
                      disabled={!t.totalAmount || suspended}
      ...
      <div>
        ...
        {payouts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 gap-3 text-slate-400 bg-slate-50 border border-slate-200 rounded-xl">
            <Wallet className="w-8 h-8 text-slate-300" />
            <p className="text-sm">No payouts found for this election.</p>
          </div>
        ) : (
